...
/**
 * Request a list of comments by post id
 * @param post_id
...
/**
 * Request a count of comments by post id
 * @param post_id
 * @returns amout of comments on a post
 */
const fetchCommentsCountByPostId = async (post_id: number): Promise<number> => {
  const [rows] = await promisePool.execute<
    RowDataPacket[] & {commentsCount: number}[]
  >('SELECT COUNT(*) as commentsCount FROM Comments WHERE post_id = ?', [post_id]);
...
const deleteComment = async (
  ...
  const params = user_level === 'Admin' ? [comment_id] : [comment_id, user_id];
